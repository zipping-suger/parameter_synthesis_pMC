...
# Example of Knuth_Yao algorithm， Using penalty CCP, penalty SCP and new linearized formulation
from GRB_optimize import gurobi_dc, gurobi_scp, gurobi_bilinear, gurobi_lp
from model_check import model_check_PRISM, model_check_LP
...
def penalty_CCP(initial_point, c_lambda=3 / 20):
    ...
    while True:
        ...
        # Model Checking
        opt_vars = model_check_LP(opt_vars)
...
def penalty_SCP(initial_point, c_lambda=3 / 20, gamma=1.5, initial_delta=2, omega=10e-4):
    ...
    while True:
        ...
        # Model Checking
        check_vars = model_check_LP(opt_vars)
...
def iteration_LP(initial_point, c_lambda=3 / 20, gamma=1.5, initial_delta=2, omega=10e-4):
    ...
    while True:
        ...
        if check_vars['ps0'] <= c_lambda:
            opt_vars = check_vars
            print("\n======\nSolution found by penalty iteration linearization")
            print("x: %f\ny; %f" % (opt_vars['x'], opt_vars['y']))
            print("The probability value is {}\n======\n".format(opt_vars['ps0']))
            return opt_vars
...
if __name__ == '__main__':
    # initialize
    init = {'x': 0.4, 'y': 0.8, 'ps0': 0.5, 'ps1': 0.3, 'ps3': 0.9}  # initial guess
    c_lambda = 0.01
    penalty_SCP(initial_point=init, c_lambda=c_lambda, initial_delta=100)
    penalty_CCP(initial_point=init, c_lambda=c_lambda)
    iteration_LP(initial_point=init, c_lambda=c_lambda,initial_delta=100)
